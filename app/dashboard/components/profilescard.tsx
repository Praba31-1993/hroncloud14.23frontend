...
export default function ProfilesCard() {
  ...
  useEffect(() => {
    ...
    try {
      userId = rememberedUser
        ? JSON.parse(rememberedUser)?.EmpId
        : rememberedUser;
    } catch {
  ...
  const renderVacationColumn = (
    ...
    <div className="listofholidays">
      ...
      <p
        className="para pt-2 textheader mb-0 shade"
    ...
    <>
      ...
      {showVacation.length > 0 && showVacation[0].showVacation && (
        <div className="d-flex gap-3 mt-2">
          <div
            className="d-flex hiddenoverflow holidays gap-5"
            style={{ width: "80%", overflowX: "auto" }}
          >
            {/* PTO Section */}
            {showVacation[0].ptoRequest && (
              <>
                {renderVacationColumn(
                  "Eligible PTO",
                  showVacation[0].empEligPaidLeaves,
                  "#a2f378",
                  AvTimerIcon
                )}
                {renderVacationColumn(
                  "Accrued PTO",
                  showVacation[0].empAccrued_PaidLeaves,
                  "#DAC5FF",
                  HourglassBottomOutlinedIcon
                )}
                {renderVacationColumn(
                  "Used PTO",
                  showVacation[0].empUsedPaidLeaves,
                  "#FFC890",
                  HourglassTopIcon
                )}
                {renderVacationColumn(
                  "Balance PTO",
                  showVacation[0].empBalancePto,
                  "#7f87eb9e",
                  BeachAccessIcon
                )}
              </>
            ...
            {showVacation[0].casualRequest && (
              <>
                {renderVacationColumn(
                  "Eligible CL",
                  showVacation[0].empEligCasualLeaves,
                  "#a2f378",
                  AirlineSeatIndividualSuiteOutlinedIcon
                )}
                {renderVacationColumn(
                  "Accrued CL",
                  showVacation?.[0]?.empAccrued_CasualLeaves,
                  "#DAC5FF",
                  AirlineSeatIndividualSuiteOutlinedIcon
                )}
                {renderVacationColumn(
                  "Used CL",
                  showVacation[0].empUsedCasualLeaves,
                  "#FFC890",
                  AirlineSeatIndividualSuiteOutlinedIcon
                )}
                {renderVacationColumn(
                  "Balance CL",
                  showVacation[0].empBalanceCL,
                  "#7f87eb9e",
                  AirlineSeatIndividualSuiteOutlinedIcon
                )}
              </>
            ...
            {showVacation[0].sickRequest && (
              <>
                {renderVacationColumn(
                  "Eligible Sick Leave",
                  showVacation[0].empEligSickLeaves,
                  "#a2f378",
                  SickIcon
                )}
                {renderVacationColumn(
                  "Accrued Sick Leave",
                  showVacation[0].empAccured_SickLeave,
                  "#DAC5FF",
                  SickIcon
                )}
                {renderVacationColumn(
            ...
            {renderVacationColumn(
              "Total Holidays",
              showVacation?.[0]?.empTotalHolidays,
              "#b3ef6d",
              LuggageOutlinedIcon
            )}
            {renderVacationColumn(
              "Used Holidays",
              showVacation?.[0]?.empUsedHolidays,
              "#7acdf5",
              LuggageIcon
            )}
          </div>
          ...
          <div>
            ...
            <p
              className="para pt-2 mb-0 shade"
              style={{ color: useColors.themeRed }}
            >
              Important Contact
